...
export class ParameterProcessor {
  ...
  public static async processParameters(text: string, currentDepth = 0): Promise<string> {
    ...
    const MAX_PARAM_RECURSION_DEPTH = VSCodeEnvironment.getConfiguration(
      'inlined-copy',
    ...
    if (currentDepth > MAX_PARAM_RECURSION_DEPTH) {
      LogManager.debug(
        `Recursion depth ${currentDepth} exceeds maximum ${MAX_PARAM_RECURSION_DEPTH}, returning text as is.`
      );
      return text;
    }
   ...
   */
  private static async promptForParameterValue(
    paramName: string,
    defaultValue: string
  ): Promise<string> {
